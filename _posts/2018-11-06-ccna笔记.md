...
A. Forwarding B. Enabled C. Disabled D. errdisabled
Correct Answer: C

378、Which statement about spanning-tree root-bridge election is true?
A. It is always performed automatically B. Each VLAN must have its own root bridge C. Each VLAN must use the same root bridge
...
q4注意全局下 routr ospf x 里面的router-id 要不同别的没啥了
